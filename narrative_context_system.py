...
CRITICAL ADDITION: Now includes ConcreteDetailTracker to maintain perfect consistency
of specific details like car models, brands, clothing, etc. across all scenes.
"""
...
class NarrativeContextManager:
    ...
    def __init__(self, session_id: str, storage_directory: Path):
        ...
        # CRITICAL: Concrete detail tracker for perfect consistency
        self.detail_tracker = ConcreteDetailTracker(session_id, storage_directory)
        ...
        # Best-effort: de-dupe persistent INFO_LEARNED logging for major narrative facts
        self._last_logged_major_fact_key: Optional[str] = None
    ...
    def add_narrative_event(self, 
        ...
        # Best-effort: persist major plot facts/revelations into everlasting ContextStore
        try:
            if ContextStore is not None:
                is_major = importance in (NarrativeImportance.CRITICAL, NarrativeImportance.IMPORTANT)
                is_fact_type = event_type in (
                    NarrativeEventType.PLOT_ADVANCEMENT,
                    NarrativeEventType.TENSION_ESCALATION,
                    NarrativeEventType.TENSION_RESOLUTION,
                    NarrativeEventType.CHARACTER_IDENTITY_DISCOVERY,
                )

                # Heuristic: dialogue exchanges can contain explicit confessions/revelations
                looks_like_revelation = False
                try:
                    if event_type == NarrativeEventType.DIALOGUE_EXCHANGE:
                        t = (narrative_text or "").lower()
                        cues = [
                            "confess", "confesses", "confession",
                            "admit", "admits", "admitted",
                            "reveal", "reveals", "revealed",
                            "tells you", "told you", "tells the truth", "the truth is",
                            "secret", "secrets", "finally tells",
                            "whispers", "leans in and says",
                            "i did it", "i did this", "i was the one",
                        ]
                        looks_like_revelation = any(c in t for c in cues)
                except Exception:
                    looks_like_revelation = False

                is_fact_type = is_fact_type or looks_like_revelation

                if is_major and is_fact_type:
                    ...
                    actor_ids: List[str] = []
                    actor_names: List[str] = [a for a in (actors_involved or []) if a]
                    try:
                        if get_spatial_manager is not None:
                            spatial = get_spatial_manager()
                            ...
                            if ctx and getattr(ctx, 'actor_positions', None):
                                for aname in actor_names:
                                    ...
                                    for aid, apos in ctx.actor_positions.items():
                                        if getattr(apos, 'actor_name', None) == aname:
                                            found = str(aid)
                                            break
                                    actor_ids.append(found or aname)
                            else:
                                actor_ids = [str(a) for a in actor_names]
                        else:
                            actor_ids = [str(a) for a in actor_names]
                    except Exception:
                        actor_ids = [str(a) for a in actor_names]
                    ...
                    excerpt = (event.summary or (narrative_text or ""))
                    excerpt = excerpt.strip().replace("\n", " ")[:180]
                    fact_key = f"{event_type.value}||{importance.value}||{excerpt}||{','.join(actor_names)}"
                    if fact_key != getattr(self, '_last_logged_major_fact_key', None):
                        self._last_logged_major_fact_key = fact_key

                        store = ContextStore(Path('simulation_data/context/context.db'))
                        summary = f"INFO LEARNED: {event.summary}"
                        payload = {
                            'actor_ids': actor_ids,
                            'actor_names': actor_names,
                            'narrative_event_type': event_type.value,
                            'narrative_importance': importance.value,
                            'summary': event.summary,
                            'full_narrative': narrative_text,
                            'emotional_tone': emotional_tone,
                            'plot_threads': list(event.plot_threads or []),
                            'relationship_changes': dict(event.relationship_changes or {}),
                        }
                        event_id = store.log_world_event(
                            ...
                            summary=summary,
                            importance=8 if importance == NarrativeImportance.CRITICAL else 7,
                            tags=['info', 'plot', 'revelation', event_type.value],
                            payload=payload,
                            world_time=wt
                        )
                            ...
                            if hasattr(store, 'remember'):
                                for aid in actor_ids:
                                    store.remember(
                                        ...
                                        content=summary,
                                        importance=8 if importance == NarrativeImportance.CRITICAL else 7,
                                        pinned=(importance == NarrativeImportance.CRITICAL),
                                        decay_rate=0.00012,
    ...
    def get_narrative_context_for_llm(self, 
                                    ...
                                    active_actors: List[str] = None) -> str:
        ...
        # CRITICAL: Add concrete details first - these MUST be maintained
        if active_actors:
            concrete_details = self.detail_tracker.get_all_active_details_context(scene_id, recent_owners=active_actors)
            if concrete_details:
                context_parts.append(concrete_details)
    ...
    def get_context_for_llm(self, lookback_events: int = 5, importance_threshold: str = "notable", 
                            use_all_events: bool = False) -> str:
        """
        Generate intelligent narrative context for LLM prompts
        
        Args:
            lookback_events: Number of recent events to consider (ignored if use_all_events=True)
            importance_threshold: Minimum importance level to include
            use_all_events: If True, searches ALL events regardless of lookback limit
        """
        try:
            ...
            # Choose event pool based on use_all_events flag
            event_pool = reversed(self.events) if use_all_events else reversed(self.events[-lookback_events:])
    ...
    def add_concrete_detail(self, category: str, owner: str, detail_text: str, 
                           keywords: List[str], scene_id: str = "current"):
        """
        Add a concrete detail that must remain consistent.
        
        Args:
            category: Category of detail (vehicle, clothing, weapon, brand, etc.)
        ...
        """
        try:
            category_enum = DetailCategory[category.upper()]
            self.detail_tracker.add_detail(category_enum, owner, detail_text, keywords, scene_id)

            # Best-effort: persist as everlasting context + seed long-term memory
            try:
                if ContextStore is not None:
                    from pathlib import Path
                    store = ContextStore(Path("simulation_data/context/context.db"))

                    # Resolve actor_id for owner (if owner matches an actor in spatial context)
                    owner_id = owner
                    try:
                        if get_spatial_manager is not None:
                            spatial = get_spatial_manager()
                            ...
                            if ctx and getattr(ctx, 'actor_positions', None):
                                for aid, apos in ctx.actor_positions.items():
                                    if getattr(apos, 'actor_name', None) == owner:
                                        owner_id = aid
                                        break
                    except Exception:
                        owner_id = owner
                    ...
                    summary = f"Concrete detail established for {owner}: {category.lower()} - {detail_text}"
                    session_id = self.session_id
                    try:
                        if 'spatial' in locals() and spatial is not None:
                            session_id = getattr(spatial, 'session_id', session_id)
                    except Exception:
                        session_id = self.session_id
                    ...
                    event_id = store.log_world_event(
                        session_id=session_id,
                        location_id=location_id,
                        event_type="DETAIL_ESTABLISHED",
                        summary=summary,
                        importance=7,
                        tags=["detail", "concrete", category.lower()],
                        payload={
                            "actor_id": owner_id,
                            "actor_ids": [owner_id],
                            "actor_name": owner,
                            "actor_names": [owner],
                            "detail_category": category.lower(),
                    ...
                    try:
                        if hasattr(store, 'remember'):
                            store.remember(
                                ...
                                memory_type="detail_established",
                                content=summary,
                                importance=7,
                                pinned=True,
                                source_event_id=int(event_id) if event_id is not None else None,
            ...
            self.logger.warning(f"Invalid detail category: {category}")
    
    def get_concrete_details_for_actor(self, actor_name: str, scene_id: str = "current") -> str:
        """
        Get all concrete details for an actor formatted for LLM context.
        ...
        Returns:
            Formatted context string with all concrete details
        """
        return self.detail_tracker.get_context_for_llm(actor_name, scene_id)
